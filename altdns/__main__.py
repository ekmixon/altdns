         ...
         for line in fp:
            ...
            for word in range(10):
               for index, value in enumerate(current_sub):
                  ...
                  current_sub[index] = f"{current_sub[index]}-{str(word)}"
                  # join the list to make into actual subdomain (aa.bb.cc)
                  actual_sub = ".".join(current_sub)
                  # save full URL as line in file
         ...
         for line in fp:
            ...
            for word in alteration_words:
               for index, value in enumerate(current_sub):
                  ...
                  current_sub[index] = (f"{word.strip()}-" + current_sub[index])
                  actual_sub = ".".join(current_sub)
...
def get_cname(q, target, resolved_out):
   ...
   try:
      result.extend(
          rdata.target for rdata in resolver.query(final_hostname, 'CNAME'))
   except:
       pass
   if len(result) == 1:
      try:
         ...
         if len(A) > 0:
            result = []
            result.extend((final_hostname, str(A[0])))
      except:
        pass
   if len(result) > 1: #will always have 1 item (target)
      ...
      resolved_out.write(f"{str(result[0])}:{str(result[1])}" + "\n")
      resolved_out.flush()
      ext = tldextract.extract(str(result[1]))
      if ext.domain == "amazonaws":
         try:
            result.extend(rdata.target for rdata in resolver.query(result[1], 'CNAME'))
         except:
             pass
      print(
          colored(
...
def main():
   ...
   if args.ignore_existing == True:
      args.output_tmp = f'{args.output}.tmp'
   else:
   ...
   if args.resolve:
      ...
      with open(args.output, "r") as fp:
         for i in fp:
            if args.threads and len(threadhandler) > int(args.threads):
